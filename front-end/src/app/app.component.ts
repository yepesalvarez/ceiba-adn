...
export class AppComponent implements OnInit {
  ...
  public errorIngreso = 'El vehiculo no se encuentra autorizado para ingresar, verificar si ya esta ingresado o incumples pico y placa';
  ...
  cobrarParqueo(idVehiculo: number) {
    ...
    this._cobroService.obtenerValorCobro(idVehiculo, fechaHoraActual).subscribe(
      response => {
  ...
  pagarParqueo(idVehiculo: number) {
    this.limpiarMensajes();
    this._parqueaderoService.pagarParqueadero(idVehiculo).subscribe(
      response => {
  ...
  retirarVehiculo(idVehiculo : number){
    this.limpiarMensajes();
    this._parqueaderoService.retirarVehiculo(idVehiculo).subscribe(
      response => {
        this.vehiculos = response;
      }, error => {
        const errorMensaje = <any> error;
